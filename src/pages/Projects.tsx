...
const Projects = () => {
  ...
  const handleViewProperties = async (projectId: number) => {
        ...
        <div className="max-w-7xl mx-auto px-6">
          ...
          {selectedProject ? (
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              <div className="bg-white rounded-3xl shadow-sm overflow-hidden mb-12 border border-slate-100">
                <div className="h-64 md:h-96 w-full relative">
                  {selectedProject.image ? (
                    ...
                    <div className="w-full h-full bg-slate-100 flex items-center justify-center">
                      <Square className="w-20 h-20 text-slate-300" />
                    </div>
                  )}
                  <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent flex flex-col justify-end p-8 md:p-12">
                    <div className={`px-4 py-1.5 rounded-full text-white text-sm font-semibold w-fit mb-4 ${statusColors[selectedProject.status]?.bg || 'bg-blue-500'}`}>
                      {statusColors[selectedProject.status]?.text || 'Pre-Selling'}
                    </div>
                ...
                <div className="p-8 md:p-12">
                  <div className="max-w-3xl">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-6 py-6 border-y border-slate-100 mb-10">
                      {selectedProject.priceRange && (
                        <div>
                          <p className="text-sm text-slate-500 mb-1">Price Range</p>
                          <p className="font-bold text-slate-900">{selectedProject.priceRange}</p>
                        </div>
                      )}
                      {selectedProject.bedRange && (
                        <div>
                          <p className="text-sm text-slate-500 mb-1">Bedrooms</p>
                          <p className="font-bold text-slate-900">{selectedProject.bedRange}</p>
                        </div>
                      )}
                      {selectedProject.bathRange && (
                        <div>
                          <p className="text-sm text-slate-500 mb-1">Bathrooms</p>
                          <p className="font-bold text-slate-900">{selectedProject.bathRange}</p>
                        </div>
                      )}
                      {selectedProject.sqftRange && (
                        <div>
                          <p className="text-sm text-slate-500 mb-1">Space</p>
                          <p className="font-bold text-slate-900">{selectedProject.sqftRange}</p>
                        </div>
                      )}
                    </div>
                  </div>

                  {/* Built-in Properties Grid */}
                  <div>
                    <div className="flex items-center justify-between mb-8">
                      <h2 className="text-3xl font-display font-bold text-slate-900">
                        Available Properties <span className="text-primary">({selectedProject.properties.length})</span>
                      </h2>
                    </div>
                    ...
                    {selectedProject.properties.length > 0 ? (
                      ...
                      <div className="text-center py-16 bg-slate-50 rounded-2xl border border-dashed border-slate-200">
                        ...
                        <p className="text-slate-500">Properties for this project will appear here once they are added.</p>
                      </div>
                    )}
                  </div>
                </div>
            ...
            <div className="animate-in fade-in duration-500">
              ...
              {/* Empty State */}
              {!loading && !loadingProject && filteredProjects.length === 0 && (
